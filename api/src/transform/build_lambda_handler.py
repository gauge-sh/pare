...
try:
    ...
    raise FileNotFoundError("Could not find required 'template.py' to build lambda handlers.") from e
...
def build_lambda_handler(symbol_path: str, output_path: Path):
    try:
        ...
        raise ValueError(f"Could not resolve module path and target symbol from: '{symbol_path}'")
